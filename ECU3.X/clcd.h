...
#define CLCD_EN				RC2   // enable pin of LCD
...
#define LINE1(x)				(0x80 + (x))   // set cursor position for line 1
#define LINE2(x)				(0xC0 + (x))   // set cursor position for line 2

#define TWO_LINE_5x8_MATRIX_8_BIT	clcd_write(0x38, INSTRUCTION_COMMAND)   // initialize LCD mode
#define CLEAR_DISP_SCREEN		clcd_write(0x01, INSTRUCTION_COMMAND)   // clear display screen
#define CURSOR_HOME			clcd_write(0x02, INSTRUCTION_COMMAND)   // move cursor to home
#define DISP_ON_AND_CURSOR_OFF	clcd_write(0x0C, INSTRUCTION_COMMAND)   // display ON and cursor OFF

void init_clcd(void);   // function to initialize LCD
void clcd_print(const unsigned char *data, unsigned char addr);   // print string on LCD
void clcd_putch(const unsigned char data, unsigned char addr);   // print one character
void clcd_write(unsigned char bit_values, unsigned char control_bit);   // send command or data to LCD
