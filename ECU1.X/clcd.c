...
void clcd_write(unsigned char byte, unsigned char control_bit)   // function to send data or command
{
	CLCD_RS = control_bit;   // select data or instruction
	CLCD_PORT = byte;   // send data to LCD port
	...
	PORT_DIR = OUTPUT;   // set port as output
}
...
void clcd_putch(const unsigned char data, unsigned char addr)   // function to print one character
{
	clcd_write(addr, INSTRUCTION_COMMAND);   // set cursor position
